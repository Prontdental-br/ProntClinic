...
const initialState: KanbanType = {
  ...
  labels: [
    ...
    { id: '5', name: 'No Laboratório', color: 'warning', notEditable: true },
    { id: '6', name: 'Na Clinica', color: 'info', notEditable: true },
    { id: '7', name: 'Concluido', color: 'success', notEditable: true },
...
interface ProsthesisContextType {
    state: KanbanType;
    ...
    filterTasksByPeriod: (period: 'all' | 'day' | 'week' | 'month') => void;
  }

  const ProsthesisContext = createContext<ProsthesisContextType | undefined>(undefined);

  export const ProsthesisProvider = ({ children }: { children: any }) => {
    const fixedLabels = [
     { id: '1', name: 'Urgente', color: 'error', notEditable: true },
    { id: '2', name: 'Importante', color: 'warning', notEditable: true },
    { id: '3', name: 'Informativo', color: 'info', notEditable: true },
 ...
 const fetchColumns = async () => {
    try {
      const response = await api.get('/prosthesis/columns');
      setState(prev => ({
        ...prev,
  ...
  const fetchLabels = async () => {
    try {
      const response = await api.get('/prosthesis/labels');
  ...
  const addColumn = async (title: string) => {
    try {
       await api.post('/prosthesis/columns', { title });
  ...
  const editColumn = async ({ id, title }: { id: string; title: string }) => {
    try {
      await api.patch(`/prosthesis/columns/${id}`, { title });
  ...
  const updateColumnOrder = async (columns: { id: string; order: number }[]) => {
    try {
      await api.put('/prosthesis/columns/order', { columns });
    } catch (error) {
  ...
  const updateTaskOrder = async (columnId: string, tasks: { id: string; order: number }[]) => {
      ...
      await api.put(`/prosthesis/tasks/order`, payload);
  ...
  const moveTask = async (columnId: string, tasks: { id: string; order: number }[]) => {
    try {
        ...
        await api.put(`/prosthesis/tasks/move`, payload);
  ...
  const deleteColumn = async (id: any) => {
    try {
      await api.delete(`/prosthesis/columns/${id}`);
      setState(prev => ({
        ...prev,
  ...
  const addTask = async (taskData: any) => {
    try {
       const newTask = await api.post('/prosthesis/tasks', taskData);
  ...
  const editTask = async (taskData: any) => {
    try {
      ...
      await api.patch(`/prosthesis/tasks/${id}`, updateData);
      setState(prev => ({
        ...prev,
  ...
  const deleteTask = async (taskId: string) => {
    try {
      await api.delete(`/prosthesis/tasks/${taskId}`);
      setState(prev => ({
        ...prev,
  ...
  const addLabel = async ({ name, color }: { name: string; color?: string }) => {
    try {
      const response = await api.post('/prosthesis/labels', { name, color });
  ...
  const editLabel = ({id, name, color}: { id: string, name: string, color: any }) => {
    try {
      api.patch(`/prosthesis/labels/${id}`, { name, color });
  ...
  const deleteLabel = async (id: string) => {
    try {
      await api.delete(`/prosthesis/labels/${id}`);
...
const filterTasksByPeriod = (period: 'all' | 'day' | 'week' | 'month') => {
  const now = dayjs();
  ...
  setState(prev => {
    ...
    const filteredTasks = prev.allTasks.filter(task => {
      const createdAt = dayjs(task.created_at);
      if (period === 'day') return createdAt.isSame(now, 'day');
      if (period === 'week') return createdAt.isSame(now, 'week');
      if (period === 'month') return createdAt.isSame(now, 'month');
...
return true;
  ...
  return (
    <ProsthesisContext.Provider value={{ 
        state, 
        updateColumnOrder, 
        addColumn, 
        editColumn, 
...
export const useProsthesis = () => {
  const context = useContext(ProsthesisContext);
  if (!context) {
    throw new Error("useProsthesis must be used within a ProsthesisProvider");
  }
