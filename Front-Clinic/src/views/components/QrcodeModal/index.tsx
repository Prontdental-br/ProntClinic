...
const QrcodeModal = ({ open, onClose, whatsAppId }: Props) => {
  ...
  const socketManager: any = useContext(SocketContext)
  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      try {
        const { data } = await prontChatApi.get(`/whatsapp/${whatsAppId}`)
  ...
  useEffect(() => {
    ...
    socket.on(`company-${companyId}-whatsappSession`, (data: any) => {
        ...
        <Paper elevation={0} style={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
          <div style={{ marginRight: '20px' }}>
            <Typography
              ...
              color='textPrimary'
              gutterBottom
              style={{ fontFamily: 'Montserrat', fontWeight: 'bold', fontSize: '20px' }}
            >
            ...
            <Typography variant='body1' color='textPrimary' gutterBottom>
              ...
              <svg width="20px" height="20px" className='MuiSvgIcon-root' focusable='false' viewBox='0 0 24 24' aria-hidden='true'>
          ...
          <div>{qrCode ? <QRCodeSVG value={qrCode} size={256} /> : <span>Waiting for QR Code</span>}</div>
        </Paper>
      </DialogContent>
    </Dialog>
  )
