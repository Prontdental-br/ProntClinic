...
import salesContext from '../scheduleContext'
...
Passando aqui para lembrar do seu agendamento conosco na %Nome da Clínica%.
Estamos te esperando com todo carinho! Caso precise remarcar, é só nos avisar por aqui`)
    ...
    <Box sx={{  width: "100%", height: "100%", padding: "20px" }}>
      ...
      {<Box sx={{ display: 'flex', marginRight: 1 }}>
            ...
            <Tooltip title='Compartilhar no WhatsApp'>
              <IconButton
                size='small'
                href={`https://wa.me/55${clearNumber(context.chanceToEdit?.phone ? context.chanceToEdit?.phone : '')}?text=${encodeURI(`Acesse seu agendamento: ${process.env.NEXT_PUBLIC_URL_FRONT}/budget/print/${context.chanceToEdit?.id}`)}`}
                component={Link}
                target="_blank"
              >
