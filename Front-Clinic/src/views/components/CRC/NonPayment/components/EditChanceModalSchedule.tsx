...
function ModalBody() {
  const context = useContext(nonPaymentContext)
   ...
   const [note, setNote] = useState(context.chanceToEdit?.observationCRC || `Olá, ${context.chanceToEdit?.author}, tudo bem?
Identificamos um valor em aberto no valor de ${context.chanceToEdit?.amount?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} referente aos seus atendimentos na(o) %Nome da Clínica%.
Se precisar de ajuda para regularizar ou deseja conversar sobre formas de pagamento, nossa equipe está à disposição para te ajudar.
...
function ModalSidebar() {
  ...
  return (
    ...
    {(context.chanceToEdit?.phone || context.chanceToEdit?.id) && (
  <Box sx={{ display: 'flex', marginRight: 1 }}>
    {context.chanceToEdit?.id && (
