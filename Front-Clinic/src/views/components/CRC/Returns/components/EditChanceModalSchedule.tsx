...
import salesContext from '../returnsContext'
...
import { Cake, CalendarMonthOutlined, WhatsApp } from '@mui/icons-material'
...
function ModalBody() {
  ...
  const [note, setNote] = useState(
    context.chanceToEdit?.observationCRC ||
      `Olá, ${context.chanceToEdit?.title}! Como vai?
Esperamos que você esteja bem após o seu procedimento realizado conosco. ✨
Chegou o momento ideal para avaliarmos seus resultados e garantir que tudo continue perfeito.

Nossa equipe da [Nome da Clínica] já está à disposição para agendar seu retorno.
Que tal marcar sua consulta e manter o cuidado em dia?

Conte sempre com a gente para cuidar do seu bem-estar!`
  )
  ...
  const handleSaveNote = async () => {
    try {
      setLoading(true)
      await api.patch(`/patients/${context.chanceToEdit?.id}`, {
        observationCRC: note
      })
      setLoading(false)
  ...
  console.log('DATA------', context.chanceToEdit.date)
    ...
    <>
      <DialogTitle>
        <Stack direction='row' style={{ alignItems: 'center' }}>
          <Box
            ...
            mr={3}
          >
            {context.chanceToEdit?.type == 'opportunity' ? <RocketLaunchIcon /> : <Cake />}
          </Box>
          <Stack>
            <Typography variant='h6'>{context.chanceToEdit?.title}</Typography>
            <Typography component='p'>
              Data Prevista para retorno:{' '}
              <b>{context.chanceToEdit?.date ? format(new Date(context.chanceToEdit.date), 'dd/MM/yyyy') : '-'}</b>
            </Typography>
          </Stack>
        </Stack>
      </DialogTitle>
      <DialogContent>
        <Box mt={4}>
          ...
          />
          <Button onClick={handleSaveNote} variant='contained' sx={{ mt: 2 }} disabled={loading || !note.trim()}>
    ...
    <Box sx={{ width: '100%', height: '100%', padding: '20px' }}>
      ...
      <Stack>
        <Stack direction='row' alignItems='start' mb={2}>
          <PersonIcon />
          <p style={{ marginTop: 0, marginBottom: 0, paddingLeft: '8px' }}>{context.chanceToEdit?.title}</p>
        </Stack>
        <Stack direction='row' alignItems='start' mb={2}>
          <NotesIcon />
          <p style={{ marginTop: 0, marginBottom: 0, paddingLeft: '8px' }}>{context.chanceToEdit?.description}</p>
        </Stack>
      </Stack>
      {
        <Box sx={{ display: 'flex', marginRight: 1 }}>
          <Tooltip title='Ficha paciente'>
            <IconButton size='small' href={`/patient/view/about/${context.chanceToEdit?.id}`} target='_blank'>
...
function CommentsModule() {
  ...
  return (
    <>
      {context.chanceToEdit?.type === 'opportunity' && (
