...
import ProsthesisTaskCard from './ProsthesisTaskCard'
import ProsthesisNewTask from './ProsthesisNewTask'
...
const ProsthesisKanbanList = (props: KanbanListProps) => {
  const { addTask, editColumn, deleteColumn, state, updateTaskOrder } = useProsthesis();
  ...
  useEffect(() => {
    ...
    return (
      <div ref={tasksListRef as RefObject<HTMLDivElement>} style={{ display: 'flex', flexDirection: 'column', width: '16.5rem' }}>
        {editDisplay ? (
          ...
          <div id="no-drag" className={styles.kanbanColumn}  style={{
            ...
            justifyContent: 'space-between',
             backgroundColor: column?.color || '#087a64',
            inlineSize: '16.5rem',
            padding: '1.2rem',
            borderRadius: '0.5rem',
            blockSize: '2.125rem',
          ...
          <Typography noWrap sx={{ color: '#F8f8f8' }}>
            {column.title}
          </Typography>
          {column.editable && (
        ...
        <ProsthesisNewTask addTask={addNewTask} />
      </div>
    )
}


export default ProsthesisKanbanList