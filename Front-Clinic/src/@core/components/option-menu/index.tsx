...
const MenuItemWrapper = ({ children, option }: { children: ReactNode; option: OptionMenuItemType }) => {
  if (option.href) {
    ...
    return <div onClick={option.onClick}>{children}</div>
  }
}

const OptionsMenu = (props: OptionsMenuType) => {
  // ** Props
  const { icon, options, menuProps, iconProps, leftAlignMenu, iconButtonProps, iconColor } = props
    ...
    <>
      <IconButton aria-haspopup='true' onClick={handleClick} {...iconButtonProps}>
         {icon ? (
            ...
            <Icon icon='mdi:dots-vertical' {...iconProps} style={{ color: iconColor ?? 'inherit' }} />
          )}
      </IconButton>
      <Menu
        keepMounted
