...
export default function EditChanceModal(props: any) {
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogActions>
        <Button style={{ marginRight: '2px' }} onClick={props.closeDialog}>
          Fechar
        </Button>
...
function ModalBody() {
 ...
 const [note, setNote] = useState(context.chanceToEdit?.observationCRC || `Olá, ${context.chanceToEdit?.author}! Esperamos que esteja tudo ótimo com você.
Verificamos que há um orçamento em aberto em seu nome aqui na(o) %Nome da Clínica%.
Estamos à disposição para esclarecer dúvidas ou agendar seu retorno.
Cuidar do seu sorriso é a nossa prioridade! `)
  const [loading, setLoading] = useState(false)

  const handleSaveNote = async () => {
    try {
      setLoading(true)
      await api.patch(`/budgets/${context.chanceToEdit?.id}`, {
        observationCRC: note,
      })
      setLoading(false);
      ...
      <DialogTitle>
        <Stack direction='row' style={{ alignItems: 'center' }}>
          <Box
            ...
            mr={3}
          >
            {context.chanceToEdit?.type == 'opportunity' ? <RocketLaunchIcon /> : <AttachMoneyIcon />}
          </Box>
          <Stack>
            ...
            <Typography component='p'>
              ...
              {context.chanceToEdit?.amount && <> - {context.chanceToEdit.getAmount()} </>}
              Observação: {context.chanceToEdit?.payment && context.chanceToEdit?.payment.observation}
            </Typography>
          </Stack>
        </Stack>
      </DialogTitle>
    ...
    <Box sx={{ width: "100%", height: "100%", padding: "20px" }}>
      ...
      {<Box sx={{ display: 'flex', marginRight: 1 }}>
            <Tooltip title='Visualizar'>
              ...
              href={`/budget/odont/?id=${context.chanceToEdit?.id}`}
              target="_blank"
              >
            ...
            <Tooltip title='Compartilhar no WhatsApp'>
              <IconButton
                size='small'
                href={`https://wa.me/55${clearNumber(context.chanceToEdit?.phone)}?text=${encodeURI(`Acesse seu orçamento: ${process.env.NEXT_PUBLIC_URL_FRONT}/budget/print/${context.chanceToEdit?.id}`)}`}
                component={Link}
                target="_blank"
              >
            ...
            <Tooltip title='Imprimir'>
              <IconButton
                size='small'
                href={`/budget/print/${context.chanceToEdit?.id}`}
                component={Link}
                target="_blank"
              >
