...
import { UpdateTaskOrderDtoOpportunity } from './dto/update-task-order';
import { UpdateColumnOrderDtoOpportunity } from './dto/update-column-order';

@UseGuards(TenantGuard)
@UseGuards(AccessTokenGuard)
@Controller('opportunity')
export class OpportunityController {
  constructor(private readonly kanbanService: OpportunityService) {}

  // column
  @Post('columns')
  createColumn(@Body() body: { title: string, color: string }): Promise<OpportunityColumn>{
    return this.kanbanService.createColumn(body.title, body.color);
  }

  @Get('columns')
  getAllColumns(): Promise<OpportunityColumn[]> {
    return this.kanbanService.findAllColumns();
  }

  @Put('columns/order')
  async updateColumnOrder(
    @Body() updateColumnOrderDto: UpdateColumnOrderDtoOpportunity,
  ) {
    await Promise.all(
  ...
  @Get('columns/:id')
  getColumn(@Param('id') id: string): Promise<OpportunityColumn> {
  ...
  ): Promise<OpportunityColumn> {
    return this.kanbanService.updateColumn(id, title, color);
  }
  ...
  @Get('/tasks/:id')
  getTask(@Param('id') id: string): Promise<OpportunityTask> {
    return this.kanbanService.findTaskById(id);
  }

  @Patch('/tasks/:id')
  updateTask(@Param('id') id: string, @Body() data: Partial<OpportunityTask>) {
  ...
  async updateTaskOrder(
    @Body() updateTaskOrderDto: UpdateTaskOrderDtoOpportunity,
  ) {
